...
function doDefault($msg = '')
{
    ...
    if (
        ($referrers === -99 || ($referrers !== -99 && $referrers->recordCount() == 0))
        ...
        if ($tablestatstups->recordCount() > 0) {
            ...
            while (!$tablestatstups->EOF) {
                ...
                echo "\t\t<td>",
                    $misc->printVal($tablestatstups->fields['idx_tup_fetch'], 'int4', $shownull),
                    "</td>\n";
        ...
        if ($tablestatsio->recordCount() > 0) {
            ...
            while (!$tablestatsio->EOF) {
                ...
                echo "\t\t<td>",
                    $misc->printVal($tablestatsio->fields['toast_blks_read'], 'int4', $shownull),
                    "</td>\n";
        ...
        if ($indexstatstups->recordCount() > 0) {
            ...
            while (!$indexstatstups->EOF) {
                ...
                echo "\t\t<td>",
                    $misc->printVal($indexstatstups->fields['idx_tup_fetch'], 'int4', $shownull),
                    "</td>\n";
                echo "\t</tr>\n";
